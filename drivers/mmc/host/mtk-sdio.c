...
static const struct of_device_id sdio_of_ids[] = {
	...
	{ .compatible = "mediatek,mt8696-sdio", .data = &mt8696_compat},
	{ .compatible = "mediatek,mt8139-sdio", .data = &mt8139_compat},
	{}
};
...
static void msdc_gate_clock(struct msdc_host *host)
{
	...
	clk_disable_unprepare(host->h_clk_cg);
...
static void msdc_ungate_clock(struct msdc_host *host)
{
	...
	clk_prepare_enable(host->h_clk_cg);
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	/*h clock control gate is optional clock*/
	host->h_clk_cg = devm_clk_get(&pdev->dev, "hclk_cg");
	if (IS_ERR(host->h_clk_cg))
		host->h_clk_cg = NULL;
...
#if defined(CONFIG_MACH_MT8168) || defined(CONFIG_MACH_MT8183)
