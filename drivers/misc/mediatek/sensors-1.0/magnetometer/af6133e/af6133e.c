...
static int af6133e_get_rawdata(int *x,int *y, int *z,int *status);
...
static int af6133e_mag_temp_comp(int16_t *mag_data)
{
 ...
 //   af6133e_t_buf[af6133e_t_index] = (int16_t)((float)af6133e_t_buf[af6133e_t_index] * TEMP_RESOLUTION) / 1000;
	...
	af6133e_t_buf[af6133e_t_index] = af6133e_t_buf[af6133e_t_index] * TEMP_RESOLUTION / 1000;
...
/*----------------------------------------------------------------------------*/
static ssize_t calidata_show(struct device_driver *ddri, char *buf)
{
  int status;
  int mag[3], tmp[3];
  char strbuf[32];
...
static ssize_t midcontrol_show(struct device_driver *ddri, char *buf)
{
  int tmp[3];
  char strbuf[32];
...
static ssize_t midcontrol_store(struct device_driver *ddri, const char *buf, size_t count)
{   
...
/*----------------------------------------------------------------------------*/
static ssize_t shipmenttest_store(struct device_driver *ddri, const char *buf, size_t count)
{
  return count;
}
/*----------------------------------------------------------------------------*/
static ssize_t shipmenttest_show(struct device_driver *ddri, char *buf)
{
  char result[10];
  int res = 0;
...
/*----------------------------------------------------------------------------*/
static ssize_t caliparas_show(struct device_driver *ddri, char *buf)
{
  int tmp[10];
  char strbuf[128];
...
/*----------------------------------------------------------------------------*/
static ssize_t bistdata_show(struct device_driver *ddri, char *buf)
{
  int tmp[9];
  char strbuf[128];
...
/*----------------------------------------------------------------------------*/
static ssize_t temp_show(struct device_driver *ddri, char *buf)
{
  int tmp[5];
  char strbuf[128];
...
/*----------------------------------------------------------------------------*/
static ssize_t registermode_show(struct device_driver *ddri, char *buf)
{
  int tmp[6];
  char strbuf[128];
  uint8_t databuf[6];
...
static DRIVER_ATTR_RO(calidata);
static DRIVER_ATTR_RO(status);
static DRIVER_ATTR_RO(caliparas);
static DRIVER_ATTR_RO(bistdata);
static DRIVER_ATTR_RO(temp);
static DRIVER_ATTR_RO(registermode);
static DRIVER_ATTR_RW(midcontrol);
static DRIVER_ATTR_RW(layout);
static DRIVER_ATTR_RW(trace);
static DRIVER_ATTR_RW(shipmenttest);
/*----------------------------------------------------------------------------*/
...
{
  /* get raw data */
  return  af6133e_get_rawdata(&data[0], &data[1], &data[2], status);
}
/*----------------------------------------------------------------------------*/
...
static int af6133e_get_rawdata(int *x,int *y, int *z,int *status)
{
  ...
  *z = databuf[2];
  ...
  printk("%s get data: %d, %d, %d. divide %d, status %d!", __func__,
...
/* prize added by xuejian, 8802 info, 20220623-start */
#ifdef CONFIG_PRIZE_HARDWARE_INFO
	strcpy(current_msensor_info.chip, "af6133e");
	sprintf(current_msensor_info.id, "0x%x", AF6133E_PID);
	strcpy(current_msensor_info.vendor, "SensorVTC");
	strcpy(current_msensor_info.more, "msensor"); 
#endif
/* prize added by xuejian, 8802 info, 20220623-end */
  MSE_LOG("%s %d: probe done\n", __func__,__LINE__);
