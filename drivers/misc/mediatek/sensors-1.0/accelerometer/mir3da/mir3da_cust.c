...
#if MIR3DA_OFFSET_TEMP_SOLUTION
/*----------------------------------------------------------------------------*/
static ssize_t calibrate_miraGSensor_show(struct device_driver *ddri, char *buf)
{
    int ret;       
...
/*----------------------------------------------------------------------------*/
static ssize_t calibrate_miraGSensor_store(struct device_driver *ddri, const char *buf, size_t count)
{
    signed char     z_dir = 0;
...
#endif 
/*----------------------------------------------------------------------------*/
static DRIVER_ATTR_RW(enable);//,          S_IRUGO | S_IWUSR,  mir3da_enable_show,             mir3da_enable_store);
static DRIVER_ATTR_RO(axis_data);//,       S_IRUGO,  mir3da_axis_data_show,          NULL);
static DRIVER_ATTR_RW(reg_data);//,        S_IWUSR | S_IRUGO,  mir3da_reg_data_show,           mir3da_reg_data_store);
static DRIVER_ATTR_RW(log_level);//,       S_IWUSR | S_IRUGO,  mir3da_log_level_show,          mir3da_log_level_store);
static DRIVER_ATTR_RO(primary_offset);//,  S_IRUGO,  mir3da_primary_offset_show,     NULL);
static DRIVER_ATTR_RO(vendor);//,          S_IRUGO,  mir3da_vendor_show,             NULL);
static DRIVER_ATTR_RO(version);//,         S_IRUGO,  mir3da_version_show,            NULL); 
#if MIR3DA_OFFSET_TEMP_SOLUTION
static DRIVER_ATTR_RW(calibrate_miraGSensor);//,       S_IWUSR | S_IRUGO,  mir3da_calibrate_show,          mir3da_calibrate_store);
#endif
#if FILTER_AVERAGE_ENHANCE
static DRIVER_ATTR_RW(average_enhance);//, S_IWUGO | S_IRUGO,  mir3da_average_enhance_show,    mir3da_average_enhance_store);
#endif 
/*----------------------------------------------------------------------------*/
...
/* prize added by chenjiaxi, 8802 info, 20220429-start */
#ifdef CONFIG_PRIZE_HARDWARE_INFO
	strcpy(current_gsensor_info.chip, "mir3da");
	sprintf(current_gsensor_info.id, "0x%x", chip_id);
	strcpy(current_gsensor_info.vendor, "MiraMEMS");
	strcpy(current_gsensor_info.more, "accelerometer"); 
#endif
/* prize added by chenjiaxi, 8802 info, 20220429-end */
    return result;
