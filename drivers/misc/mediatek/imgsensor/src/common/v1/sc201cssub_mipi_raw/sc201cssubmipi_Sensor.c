...
#include "sc201cssubmipi_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "sc201cssub_camera_sensor_JF"
#define LOG_1 LOG_INF("SC201CSSUB, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = SC201CSSUBMIPI_SENSOR_ID,
...
#if 0
static struct sc201cssub_otp sc201cssub_otp_info;

static kal_uint8 sc201cssub_read_otp(kal_uint8 addr)
{
	kal_uint8 value;
	kal_uint8 regd4;
...
static void sc201cssub_read_otp_group(kal_uint8 addr, kal_uint8 *buff, int size)
{
	kal_uint8 i;
	kal_uint8 regd4;
	kal_uint16 realaddr = addr * 8;
...
static void sc201cssub_select_page_otp(kal_uint8 otp_select_page)
{
	kal_uint8 page;
...
static void sc201cssub_gcore_read_otp_info(void)
{
	...
	memset(&sc201cssub_otp_info, 0, sizeof(sc201cssub_otp_info));

	/*TODO*/
	sc201cssub_select_page_otp(otp_page0);
	flagdd = sc201cssub_read_otp(0x00);
	LOG_INF("flag_dd = 0x%x\n", flagdd);
	flag_chipversion = sc201cssub_read_otp(0x7f);

	/* DD */
	switch (flagdd & 0x03) {
	case 0x00:
		LOG_INF("DD is empty!\n");
		sc201cssub_otp_info.dd_flag = 0x00;
		break;
	case 0x01:
		LOG_INF("DD is valid!\n");
		total_number = sc201cssub_read_otp(0x01) + sc201cssub_read_otp(0x02);
		LOG_INF("total_number = %d\n", total_number);

		if (total_number <= 31)
			sc201cssub_read_otp_group(0x03, &ddtempbuff[0], total_number * 4);
		else {
			sc201cssub_read_otp_group(0x03, &ddtempbuff[0], 31 * 4);
			sc201cssub_select_page_otp(otp_page1);
			sc201cssub_read_otp_group(0x29, &ddtempbuff[31 * 4], (total_number - 31) * 4);
		}
		...
		for (i = 0; i < total_number; i++) {
			...
			if (ddtempbuff[4 * i + 3] & 0x10) {
				...
				case 3:
					for (j = 0; j < 4; j++) {
						sc201cssub_otp_info.dd_param_x[cnt] =
							(((kal_uint16)ddtempbuff[4 * i + 1] & 0x0f) << 8)
							+ ddtempbuff[4 * i];
						sc201cssub_otp_info.dd_param_y[cnt] =
							((kal_uint16)ddtempbuff[4 * i + 2] << 4)
							+ ((ddtempbuff[4 * i + 1] & 0xf0) >> 4) + j;
						sc201cssub_otp_info.dd_param_type[cnt++] = 2;
					}
					break;
				case 4:
					for (j = 0; j < 2; j++) {
				...
				default:
					...
					sc201cssub_otp_info.dd_param_type[cnt++] = ddtempbuff[4 * i + 3] & 0x0f;
					break;
				}
			} else
		...
		sc201cssub_otp_info.dd_cnt = cnt;
		sc201cssub_otp_info.dd_flag = 0x01;
		break;
	case 0x02:
	case 0x03:
		LOG_INF("DD is invalid!\n");
		sc201cssub_otp_info.dd_flag = 0x02;
		break;
	default:
		break;
	}
	...
	case 0x01:
		...
		i = 0;
		do {
			sc201cssub_otp_info.reg_addr[i] = sc201cssub_read_otp(REG_ROM_START + i*2);
			sc201cssub_otp_info.reg_value[i] = sc201cssub_read_otp(REG_ROM_START + i * 2 + 1);
			LOG_INF("reg_addr[%d] = 0x%x, reg_value[%d] = 0x%x\n",
				i, sc201cssub_otp_info.reg_addr[i], i, sc201cssub_otp_info.reg_value[i]);
			i++;
		} while ((sc201cssub_otp_info.reg_addr[i - 1] != 0) && (i < 10));
		sc201cssub_otp_info.reg_num = i - 1;
		break;
	case 0x02:
...
#if SC201CSSUBOTP_FOR_CUSTOMER
	sc201cssub_select_page_otp(otp_page1);
	flag1 = sc201cssub_read_otp(0x00);
	flag_golden = sc201cssub_read_otp(0x1b);
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x01:
			...
			check = 0;
			sc201cssub_read_otp_group(INFO_ROM_START + index * INFO_WIDTH, &info[0], INFO_WIDTH);
			for (i = 0; i < INFO_WIDTH - 1; i++)
				check += info[i];

			if ((check % 255 + 1) == info[INFO_WIDTH-1]) {
				sc201cssub_otp_info.module_id = info[0];
				sc201cssub_otp_info.lens_id = info[1];
				sc201cssub_otp_info.vcm_driver_id = info[2];
				sc201cssub_otp_info.vcm_id = info[3];
				sc201cssub_otp_info.year = info[4];
				sc201cssub_otp_info.month = info[5];
				sc201cssub_otp_info.day = info[6];
			} else
		...
		case 0x00:
			...
			sc201cssub_otp_info.wb_flag = sc201cssub_otp_info.wb_flag | 0x00;
			break;
		case 0x01:
			LOG_INF("wb group %d is valid!\n", index + 1);
			check = 0;
			sc201cssub_read_otp_group(WB_ROM_START + index * WB_WIDTH, &wb[0], WB_WIDTH);
			for (i = 0; i < WB_WIDTH - 1; i++)
				check += wb[i];

			if ((check % 255 + 1) == wb[WB_WIDTH - 1]) {
				sc201cssub_otp_info.rg_gain = (((wb[0] << 8) & 0xff00) | wb[1]) > 0 ?
					(((wb[0] << 8) & 0xff00) | wb[1]) : 0x400;
				sc201cssub_otp_info.bg_gain = (((wb[2] << 8) & 0xff00) | wb[3]) > 0 ?
					(((wb[2] << 8) & 0xff00) | wb[3]) : 0x400;
				sc201cssub_otp_info.wb_flag = sc201cssub_otp_info.wb_flag | 0x01;
			} else
		...
		case 0x03:
			...
			sc201cssub_otp_info.wb_flag = sc201cssub_otp_info.wb_flag | 0x02;
			break;
		default:
			break;
		}
		...
		case 0x00:
			...
			sc201cssub_otp_info.golden_flag = sc201cssub_otp_info.golden_flag | 0x00;
			break;
		case 0x01:
			LOG_INF("wb golden group %d is valid!\n", index + 1);
			check = 0;
			sc201cssub_read_otp_group(GOLDEN_ROM_START + index * GOLDEN_WIDTH, &golden[0], GOLDEN_WIDTH);
			for (i = 0; i < GOLDEN_WIDTH - 1; i++)
				check += golden[i];

			if ((check % 255 + 1) == golden[GOLDEN_WIDTH - 1]) {
				sc201cssub_otp_info.golden_rg = (((golden[0] << 8) & 0xff00) | golden[1]) > 0 ?
					(((golden[0] << 8) & 0xff00) | golden[1]) : RG_TYPICAL;
				sc201cssub_otp_info.golden_bg = (((golden[2] << 8) & 0xff00) | golden[3]) > 0 ?
					(((golden[2] << 8) & 0xff00) | golden[3]) : BG_TYPICAL;
				sc201cssub_otp_info.golden_flag = sc201cssub_otp_info.golden_flag | 0x01;
			} else
		...
		case 0x03:
			...
			sc201cssub_otp_info.golden_flag = sc201cssub_otp_info.golden_flag | 0x02;
			break;
		default:
			break;
		}
	...
	LOG_INF("module_id = 0x%x\n", sc201cssub_otp_info.module_id);
	LOG_INF("lens_id = 0x%x\n", sc201cssub_otp_info.lens_id);
	LOG_INF("vcm_id = 0x%x\n", sc201cssub_otp_info.vcm_id);
	LOG_INF("vcm_driver_id = 0x%x\n", sc201cssub_otp_info.vcm_driver_id);
	LOG_INF("data = %d-%d-%d\n", sc201cssub_otp_info.year, sc201cssub_otp_info.month, sc201cssub_otp_info.day);
	LOG_INF("r/g = 0x%x\n", sc201cssub_otp_info.rg_gain);
	LOG_INF("b/g = 0x%x\n", sc201cssub_otp_info.bg_gain);
	LOG_INF("golden_rg = 0x%x\n", sc201cssub_otp_info.golden_rg);
	LOG_INF("golden_bg = 0x%x\n", sc201cssub_otp_info.golden_bg);
#endif
}

static void sc201cssub_gcore_update_dd(void)
{
	kal_uint16 i = 0, j = 0;
	...
	if (sc201cssub_otp_info.dd_flag == 0x01) {
#if SC201CSSUB_MIRROR_FLIP_ENABLE
		for (i = 0; i < sc201cssub_otp_info.dd_cnt; i++) {
			if (sc201cssub_otp_info.dd_param_type[i] == 0) {
				sc201cssub_otp_info.dd_param_x[i] = WINDOW_WIDTH - sc201cssub_otp_info.dd_param_x[i] + 1;
				sc201cssub_otp_info.dd_param_y[i] = WINDOW_HEIGHT - sc201cssub_otp_info.dd_param_y[i] + 1;
			} else if (sc201cssub_otp_info.dd_param_type[i] == 1) {
				sc201cssub_otp_info.dd_param_x[i] = WINDOW_WIDTH - sc201cssub_otp_info.dd_param_x[i] - 1;
				sc201cssub_otp_info.dd_param_y[i] = WINDOW_HEIGHT - sc201cssub_otp_info.dd_param_y[i] + 1;
			} else {
				sc201cssub_otp_info.dd_param_x[i] = WINDOW_WIDTH - sc201cssub_otp_info.dd_param_x[i];
				sc201cssub_otp_info.dd_param_y[i] = WINDOW_HEIGHT - sc201cssub_otp_info.dd_param_y[i] + 1;
			}
		}
#else
		...
		for (i = 0; i < sc201cssub_otp_info.dd_cnt - 1; i++)
			for (j = i + 1; j < sc201cssub_otp_info.dd_cnt; j++)
				if (sc201cssub_otp_info.dd_param_y[i] * WINDOW_WIDTH
					+ sc201cssub_otp_info.dd_param_x[i]
					> sc201cssub_otp_info.dd_param_y[j] * WINDOW_WIDTH
					+ sc201cssub_otp_info.dd_param_x[j]) {
					temp_x = sc201cssub_otp_info.dd_param_x[i];
					sc201cssub_otp_info.dd_param_x[i] = sc201cssub_otp_info.dd_param_x[j];
					sc201cssub_otp_info.dd_param_x[j] = temp_x;
					temp_y = sc201cssub_otp_info.dd_param_y[i];
					sc201cssub_otp_info.dd_param_y[i] = sc201cssub_otp_info.dd_param_y[j];
					sc201cssub_otp_info.dd_param_y[j] = temp_y;
					temp_type = sc201cssub_otp_info.dd_param_type[i];
					sc201cssub_otp_info.dd_param_type[i] = sc201cssub_otp_info.dd_param_type[j];
					sc201cssub_otp_info.dd_param_type[j] = temp_type;
				}
		...
		for (i = 0; i < sc201cssub_otp_info.dd_cnt; i++) {
			temp_val0 = sc201cssub_otp_info.dd_param_x[i] & 0x00ff;
			temp_val1 = (((sc201cssub_otp_info.dd_param_y[i]) << 4) & 0x00f0)
				+ ((sc201cssub_otp_info.dd_param_x[i] >> 8) & 0x000f);
			temp_val2 = (sc201cssub_otp_info.dd_param_y[i] >> 4) & 0xff;
			write_cmos_sensor(0xaa, i);
			write_cmos_sensor(0xac, temp_val0);
			write_cmos_sensor(0xac, temp_val1);
			write_cmos_sensor(0xac, temp_val2);
			while ((i < sc201cssub_otp_info.dd_cnt - 1)
				&& (sc201cssub_otp_info.dd_param_x[i] == sc201cssub_otp_info.dd_param_x[i+1])
				&& (sc201cssub_otp_info.dd_param_y[i] == sc201cssub_otp_info.dd_param_y[i+1])) {
				flag = 1;
				i++;
			}
			if (flag)
				...
				write_cmos_sensor(0xac, sc201cssub_otp_info.dd_param_type[i]);
...
#if SC201CSSUBOTP_FOR_CUSTOMER
static void sc201cssub_gcore_update_wb(void)
{
	...
	if (sc201cssub_otp_info.golden_flag == 0x02)
		return;

	if (0x00 == (sc201cssub_otp_info.golden_flag & 0x01)) {
		rg_typical = RG_TYPICAL;
		bg_typical = BG_TYPICAL;
	}

	if (0x01 == (sc201cssub_otp_info.golden_flag & 0x01)) {
		rg_typical = sc201cssub_otp_info.golden_rg;
		bg_typical = sc201cssub_otp_info.golden_bg;
		LOG_INF("rg_typical = 0x%x, bg_typical = 0x%x\n", rg_typical, bg_typical);
	}

	if (0x01 == (sc201cssub_otp_info.wb_flag & 0x01)) {
		r_gain_current = 1024 * rg_typical / sc201cssub_otp_info.rg_gain;
		b_gain_current = 1024 * bg_typical / sc201cssub_otp_info.bg_gain;
		g_gain_current = 1024;
...
static void sc201cssub_gcore_update_chipversion(void)
{
	kal_uint8 i;

	LOG_INF("reg_num = %d\n", sc201cssub_otp_info.reg_num);
	for (i = 0; i < sc201cssub_otp_info.reg_num; i++) {
		write_cmos_sensor(sc201cssub_otp_info.reg_addr[i], sc201cssub_otp_info.reg_value[i]);
		LOG_INF("{0x%x, 0x%x}\n", sc201cssub_otp_info.reg_addr[i], sc201cssub_otp_info.reg_value[i]);
	}
}

static void sc201cssub_gcore_update_otp(void)
{
	sc201cssub_gcore_update_dd();
#if SC201CSSUBOTP_FOR_CUSTOMER
	sc201cssub_gcore_update_wb();
#endif
	sc201cssub_gcore_update_chipversion();
}

static void sc201cssub_gcore_enable_otp(kal_uint8 state)
{
...
static void sc201cssub_gcore_initial_otp(void)
{
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xfe, 0x00);
...
static void sc201cssub_gcore_identify_otp(void)
{
	sc201cssub_gcore_initial_otp();
	sc201cssub_gcore_enable_otp(otp_open);
	sc201cssub_gcore_read_otp_info();
	sc201cssub_gcore_enable_otp(otp_close);
}
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (reg_gain < SC201CSSUB_SENSOR_GAIN_BASE)
		reg_gain = SC201CSSUB_SENSOR_GAIN_BASE;
	else if (reg_gain > SC201CSSUB_SENSOR_GAIN_MAX)
		reg_gain = SC201CSSUB_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 SC201CSSUB_AGC_Param[SC201CSSUB_SENSOR_GAIN_MAX_VALID_INDEX][2] = {
		{  1024,  0x00 },
		{  2048,  0x01 },
		{  4096,  0x03 },
		{  8192,  0x07 },
	...
	for (gain_index = SC201CSSUB_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index >= 0; gain_index--)
		if (reg_gain >= SC201CSSUB_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor(0x3e09, SC201CSSUB_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * SC201CSSUB_SENSOR_GAIN_BASE / SC201CSSUB_AGC_Param[gain_index][0];
	write_cmos_sensor(0x3e07, (temp_gain >> 3) & 0xff);
	
	LOG_INF("Exit! SC201CSSUB_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, gain = 0x%x, reg_gain = %d\n",
		SC201CSSUB_AGC_Param[gain_index][1], temp_gain, gain, reg_gain);
...
write_cmos_sensor(0x3221, SC201CSSUB_MIRROR);
	LOG_INF("end\n");
}
...
static kal_uint32 open(void)
{
	...
	//sc201cssub_gcore_check_version();

	/* Don't Remove!! */
	//sc201cssub_gcore_identify_otp();

	/* initail sequence write in */
	sensor_init();

	/* write registers to sram */
	//sc201cssub_gcore_update_otp();
...
UINT32 SC201CSSUBMIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
