...
#include "imx351gmsmipiraw_Sensor.h"
//drv add by lipengpeng 20230323 start 
#include <linux/of_gpio.h>
#include <linux/gpio.h>
//drv add by lipengpeng 20230323 end
#define PFX "IMX351GMS_camera_sensor"

#define LOG_1 LOG_INF("IMX351GMS,MIPI 4LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_R,//SENSOR_OUTPUT_FORMAT_RAW_Gr,//SENSOR_OUTPUT_FORMAT_RAW_R, //SENSOR_OUTPUT_FORMAT_RAW_B
	.mclk = 24,
	.mipi_lane_num = SENSOR_MIPI_4_LANE,
	.i2c_addr_table = {0x20, 0x34, 0xff}, // {0x20, 0x21, 0xff},
	.i2c_speed = 400,// i2c read/write speed
};


static imgsensor_struct imgsensor = {
	.mirror = IMAGE_NORMAL,//IMAGE_NORMAL,				//mirrorflip information //IMAGE_HV_MIRROR
...
{ 4656, 3496, 0, 4, 4656, 3488, 2328, 1744, 0000, 0000, 2328, 1744, 0, 0, 2328, 1744}, //hight speed video
{ 4656, 3496, 0, 4, 4656, 3488, 2328, 1744, 0000, 0000, 2328, 1744, 0, 0, 2328, 1744},// slim video
...
static void write_shutter(kal_uint16 shutter)
{
    ...
    write_cmos_sensor_8(0x0104, 0x00);
    LOG_INF("imx351gms Exit! shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
   ...
   LOG_INF("imx351gms imx351gms gain =%d, reg_gain =%d\n", gain,reg_gain);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    if (gain < BASEGAIN || gain > 16 * BASEGAIN) {
        LOG_INF("imx351gms Error gain setting");
    ...
    spin_unlock(&imgsensor_drv_lock);
    LOG_INF("imx351gms gain = %d , reg_gain = 0x%x\n ", gain, reg_gain);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			*sensor_id = ((read_cmos_sensor_8(0x0016) << 8) | read_cmos_sensor_8(0x0017))+0x1000;
			printk("imx351gms sensor id gpio_get_value(24)=%d (26)=%d sensor_id=0x%x\n",gpio_get_value(70),gpio_get_value(72),*sensor_id);
			LOG_INF("imx351gms read_0x0000=0x%x, 0x0001=0x%x,0x0000_0001=0x%x\n",read_cmos_sensor_8(0x0000),read_cmos_sensor_8(0x0001),read_cmos_sensor(0x0000));
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("imx351gms i2c write id: 0x%x, 351sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			
				return ERROR_NONE;
			}
			LOG_INF("imx351gms Read sensor id fail, i2c_write_id: 0x%x .351sensor_id: 0x%x \n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	LOG_INF("imx351gms PLATFORM:MT6595,MIPI 2LANE\n");
	LOG_INF("imx351gms preview 1280*960@30fps,864Mbps/lane; video 1280*960@30fps,864Mbps/lane; capture 5M@30fps,864Mbps/lane\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			gpio_direction_input(26+46);
            sensor_id = ((read_cmos_sensor_8(0x0016) << 8) | read_cmos_sensor_8(0x0017))+0x1000;
			printk("imx351gms gpio_get_value(24)=%d (26)=%d sensor id: 0x%x \n",gpio_get_value(70),gpio_get_value(72),sensor_id);
			if( gpio_get_value(70)==0)
			{
				return ERROR_SENSOR_CONNECT_FAIL;
			}
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("imx351gms i2c write id: 0x%x,open351 sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
				break;
			}
			LOG_INF("imx351gms Read sensor id fail, id: 0x%x sensor_id=0x%x\n ", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
...
{
	LOG_INF("imx219 enable: %d current_scenario_id:%d \n", enable,imgsensor.current_scenario_id);
	if(imgsensor.current_scenario_id != MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG)
	   {
		   ...
		   {   
			   ...
			   write_cmos_sensor_8(0x0624, 0x06); //W:3280---h
			   write_cmos_sensor_8(0x0625, 0x68); //		  l
			   write_cmos_sensor_8(0x0626, 0x04); //H:2464   h
			   write_cmos_sensor_8(0x0627, 0xd0); //		  l
			   write_cmos_sensor_8(0x6128, 0x00); 
			   write_cmos_sensor_8(0x6129, 0x02); 		 
			   write_cmos_sensor_8(0x613C, 0x06); //W 		h
			   write_cmos_sensor_8(0x613D, 0x68); //		   l
			   write_cmos_sensor_8(0x613E, 0x04); //H 		h
			   write_cmos_sensor_8(0x613F, 0xd0); //			   l
			   write_cmos_sensor_8(0x6506, 0x00); 
			   write_cmos_sensor_8(0x6507, 0x00);
		   } 
		   else 
		   {   
	   ...
	   else
	   {
		   ...
		   {   
			   ...
			   write_cmos_sensor_8(0x0600, 0x00); 
			   write_cmos_sensor_8(0x0601, 0x02); 	 
			   write_cmos_sensor_8(0x0624, 0x0C); //W:3280---h
			   write_cmos_sensor_8(0x0625, 0xD0); //		  l
			   write_cmos_sensor_8(0x0626, 0x09); //H:2464   h
			   write_cmos_sensor_8(0x0627, 0xA0); //		  l
			   write_cmos_sensor_8(0x6128, 0x00); 
			   write_cmos_sensor_8(0x6129, 0x02); 		 
			   write_cmos_sensor_8(0x613C, 0x0C); //W 		h
			   write_cmos_sensor_8(0x613D, 0xD0); //		   l
			   write_cmos_sensor_8(0x613E, 0x09); //H 		h
			   write_cmos_sensor_8(0x613F, 0xA0); //			   l
			   write_cmos_sensor_8(0x6506, 0x00); 
			   write_cmos_sensor_8(0x6507, 0x00);
		   } 
		   else 
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {//PIP capture: 24fps for less than 13M, 20fps for 16M,15fps for 20M
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			LOG_INF("imx351gms Warning: current_fps %d fps is not support, so use cap1's setting: %d fps!\n",imgsensor.current_fps,imgsensor_info.cap1.max_framerate/10);
...
static kal_uint32 get_info(enum MSDK_SCENARIO_ID_ENUM scenario_id,
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_INF("imx351gms scenario_id = %d\n", scenario_id);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	switch (scenario_id) {
		...
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
			...
			break;
		default:
			LOG_INF("imx351gms Error ScenarioId setting");
			preview(image_window, sensor_config_data);
...
{
	LOG_INF("imx351gms enable = %d, framerate = %d \n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	LOG_INF("imx351gms scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	switch (scenario_id) {
        ...
        case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
        	  if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
                ...
                spin_lock(&imgsensor_drv_lock);
        		    ...
        		    if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
                    LOG_INF("imx351gms Warning: current_fps %d fps is not support, so use cap's setting: %d fps!\n",framerate,imgsensor_info.cap.max_framerate/10);
		...
		default:  //coding with  preview scenario by default
			...
			spin_unlock(&imgsensor_drv_lock);
			set_dummy();
			LOG_INF("imx351gms error scenario_id = %d, we use preview scenario \n", scenario_id);
			break;
	}
	return ERROR_NONE;
}
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
            LOG_INF("imx351gms feature_Control imgsensor.pclk = %d,imgsensor.current_fps = %d\n", imgsensor.pclk,imgsensor.current_fps);
		...
		case SENSOR_FEATURE_SET_FRAMERATE:
            LOG_INF("imx351gms current fps :%d\n", (UINT32)*feature_data);
			spin_lock(&imgsensor_drv_lock);
                     imgsensor.current_fps = *feature_data;
			spin_unlock(&imgsensor_drv_lock);
			break;
		case SENSOR_FEATURE_SET_HDR:
            LOG_INF("imx351gms ihdr enable :%d\n", (BOOL)*feature_data);
			spin_lock(&imgsensor_drv_lock);
