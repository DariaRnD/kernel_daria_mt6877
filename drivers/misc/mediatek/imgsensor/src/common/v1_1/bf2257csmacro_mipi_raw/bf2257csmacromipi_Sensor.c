...
#include "bf2257csmacromipi_Sensor.h"


/****************************Modify Following Strings for Debug****************************/
#define PFX "bf2257csmacrocamera_sensor"
#define LOG_1 LOG_INF("bf2257csmacro, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_err(PFX "[%s] " format, __func__, ##args)

#define BASIC_LINE 1234
#define BF2257CSMACRO_MIRROR_FLIP_ENABLE 1

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = BF2257CSMACRO_MIPI_SENSOR_ID,
	...
	.i2c_addr_table				= {0xdc,0x7c,0xff},			//record sensor support all write id addr, only supprt 4, must end with 0xff
};
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	switch( image_mirror ) {
		case IMAGE_NORMAL:
			...
			break;
		case IMAGE_H_MIRROR:
			write_cmos_sensor( 0x01, 0x09 );
			break;
		case IMAGE_V_MIRROR:
			write_cmos_sensor( 0x01, 0x05 );
			break;
		case IMAGE_HV_MIRROR:
			write_cmos_sensor( 0x01, 0x0d );
			break;
		default:
...
static void sensor_init(void)
{
    ...
    #if BF2257CSMACRO_MIRROR_FLIP_ENABLE
    write_cmos_sensor(0x00, 0x0d);//Bit[3]: Mirror Bit[2]: vFlip
    #else
