...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV8856GMS_SENSOR_ID,		/*record sensor id defined in Kd_imgsensor.h*/
...
static kal_uint16 ov8856gms_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
...
static void sensor_init(void)
{
	LOG_INF("v2 E\n");
#if 1	/* MULTI_WRITE */
	ov8856gms_table_write_cmos_sensor(addr_data_pair_init_ov8856gms,
...
#endif
	ov8856gms_table_write_cmos_sensor(addr_data_pair_preview_ov8856gms,
...
#endif
	if (currefps == 150) {
		ov8856gms_table_write_cmos_sensor(addr_data_pair_capture_15fps_ov8856gms,
					       sizeof(addr_data_pair_capture_15fps_ov8856gms) /
					       sizeof(kal_uint16));
	} else {
		ov8856gms_table_write_cmos_sensor(addr_data_pair_capture_30fps_ov8856gms,
...
static void vga_setting_120fps(void)
{
	...
	ov8856gms_table_write_cmos_sensor(addr_data_pair_vga_setting_120fps_ov8856gms,
		...
		do {
			...
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C))+1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("ov8856gms i2c write id: 0x%x, sensor id: 0x%x, ver = 0x%x<0xB1=r2a,0xB0=r1a>\n",
					imgsensor.i2c_write_id, *sensor_id, read_cmos_sensor(0x302A));
				return ERROR_NONE;
			}
			printk("ov8856gms Read sensor id fail,write_id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
