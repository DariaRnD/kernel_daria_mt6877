...
static INT32 consys_jtag_set_for_mcu(VOID)
{
	...
	if (gJtagCtrl) {
		...
		if (remap_addr1 == 0) {
			WMT_PLAT_PR_DBG("remap jtag_addr1 fail!\n");
			ret = -1;
			goto error;
		}
		...
		if (remap_addr2 == 0) {
			WMT_PLAT_PR_DBG("remap jtag_addr2 fail!\n");
			ret = -1;
			goto error;
		}
...
error:
	...
	return ret;
#else
	WMT_PLAT_PR_DBG("No support switch to JTAG!\n");
...
static INT32 consys_cr_remapping(UINT32 enable)
{
	WMT_PLAT_PR_DBG("%s consys_cr_remapping\n", enable ? "enable" : "disable");
...
static INT32 consys_clk_get_from_dts(struct platform_device *pdev)
{
	...
	if (IS_ERR(clk_scp_conn_main)) {
		WMT_PLAT_PR_DBG("[CCF]cannot get clk_scp_conn_main clock.\n");
	...
	if (IS_ERR(clk_infracfg_ao_ccif4_ap_cg)) {
		WMT_PLAT_PR_DBG("[CCF]cannot get clk_infracfg_ao_ccif4_ap_cg clock.\n");
...
static INT32 consys_pmic_get_from_dts(struct platform_device *pdev)
{
	...
	if (!reg_VCN18)
		WMT_PLAT_PR_DBG("Regulator_get VCN_1V8 fail\n");
	reg_VCN13 = regulator_get(&pdev->dev, "vcn13");
	if (!reg_VCN13)
		WMT_PLAT_PR_DBG("Regulator_get VCN_1V3 fail\n");
	reg_VCN33_1_BT = regulator_get(&pdev->dev, "vcn33_1_bt");
	if (!reg_VCN33_1_BT)
		WMT_PLAT_PR_DBG("Regulator_get VCN33_1_BT fail\n");
	reg_VCN33_1_WIFI = regulator_get(&pdev->dev, "vcn33_1_wifi");
	if (!reg_VCN33_1_WIFI)
		WMT_PLAT_PR_DBG("Regulator_get VCN33_1_WIFI fail\n");
	reg_VCN33_2_WIFI = regulator_get(&pdev->dev, "vcn33_2_wifi");
	if (!reg_VCN33_2_WIFI)
		WMT_PLAT_PR_DBG("Regulator_get VCN33_2_WIFI fail\n");
#endif
...
static VOID consys_set_if_pinmux(MTK_WCN_BOOL enable)
{
	...
	if (!consys_if_pinmux_reg_base) {
		WMT_PLAT_PR_DBG("consys_if_pinmux_reg_base(%x) ioremap fail\n",
	...
	if (!consys_if_pinmux_driving_base) {
		WMT_PLAT_PR_DBG("consys_if_pinmux_driving_base(%x) ioremap fail\n",
	...
	if (enable) {
		...
		if (wmt_plat_soc_co_clock_flag_get() == 0) {
			...
			WMT_PLAT_PR_DBG("set GPIO137 pinmux for TCXO mode (Aux5)\n");
		}
#endif
	} else {
		CONSYS_REG_WRITE(consys_if_pinmux_reg_base + CONSYS_IF_PINMUX_01_OFFSET,
...
static INT32 consys_hw_power_ctrl(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
		...
		if (iRet) {
			WMT_PLAT_PR_DBG("clk_prepare_enable(clk_infracfg_ao_ccif4_ap_cg) fail(%d)\n", iRet);
			return iRet;
		}
		...
		if (iRet)
			WMT_PLAT_PR_DBG("pm_runtime_get_sync() fail(%d)\n", iRet);
		else
			WMT_PLAT_PR_DBG("pm_runtime_get_sync() CONSYS ok\n");

		iRet = device_init_wakeup(&connsys_pdev->dev, true);
		if (iRet)
			WMT_PLAT_PR_DBG("device_init_wakeup(true) fail.\n");
		else
			WMT_PLAT_PR_DBG("device_init_wakeup(true) CONSYS ok\n");
#else
		iRet = clk_prepare_enable(clk_scp_conn_main);
		if (iRet) {
			WMT_PLAT_PR_DBG("clk_prepare_enable(clk_scp_conn_main) fail(%d)\n", iRet);
			return iRet;
		}
		...
		if (value == 0)
			WMT_PLAT_PR_DBG("[POS polling info] >> check connsys_on_domain_pwr_ack status fail\n");
		...
		if (value == 0)
			WMT_PLAT_PR_DBG("[POS polling info] >> check connsys_on_domain_pwr_ack_s status fail\n");
		...
		if (value == 0)
			WMT_PLAT_PR_DBG("[POS polling info] >> check connsys_off_domain_pwr_ack status fail\n");
		...
		if (value == 0)
			WMT_PLAT_PR_DBG("[POS polling info] >> check AHB RX bus sleep protect turn off fail\n");
		...
		if (value == 0)
			WMT_PLAT_PR_DBG("[POS polling info] >> check AXI Rx bus sleep protect turn off fail\n");
		...
		if (value == 0)
			WMT_PLAT_PR_DBG("[POS polling info] >> check AXI Tx bus sleep protect turn off fail\n");
		...
		if (value == 0)
			WMT_PLAT_PR_DBG("[POS polling info] >> check AHB TX bus sleep protect turn off fail\n");
		...
		if (iRet)
			WMT_PLAT_PR_DBG("device_init_wakeup(false) fail.\n");
		else
			WMT_PLAT_PR_DBG("device_init_wakeup(false) CONSYS ok\n");

		iRet = pm_runtime_put_sync(&connsys_pdev->dev);
		if (iRet)
			WMT_PLAT_PR_DBG("pm_runtime_put_sync() fail.\n");
		else
			WMT_PLAT_PR_DBG("pm_runtime_put_sync() CONSYS ok\n");
#else
...
static INT32 polling_consys_chipid(VOID)
{
	...
	while (retry-- > 0) {
		...
		WMT_PLAT_PR_DBG("Read CONSYS version id(0x%08x)", consys_ver_id);
		msleep(20);
	}
...
static VOID consys_afe_reg_setting(VOID)
{
	...
	if (consys_afe_wbg_reg_base) {
		...
		WMT_PLAT_PR_DBG("CONSYS_AFE_WBG_REG_BASE(0x%x) ioremap fail!\n",
...
static VOID consys_set_xo_osc_ctrl(VOID)
{
	...
	if (consys_reg_base) {
		...
		iounmap(consys_reg_base);
			...
			WMT_PLAT_PR_DBG("connsys co_clock stable time base(0x%x) ioremap fail!\n",
...
static VOID consys_identify_adie(VOID)
{
	...
	if (consys_reg_base) {
		...
		WMT_PLAT_PR_DBG("CONSYS_IDENTIFY_ADIE_CR_ADDRESS(0x%x) ioremap fail!\n",
...
static VOID consys_wifi_ctrl_setting(VOID)
{
	...
	if (reg_base) {
		...
		WMT_PLAT_PR_DBG("consys_if_pinmux_reg_base(0x%x) ioremap fail!\n",
			CONSYS_IF_PINMUX_REG_BASE);
	}
}
...
static VOID consys_bus_timeout_config(VOID)
{
	...
	if (consys_reg_base) {
		...
		WMT_PLAT_PR_DBG("CONSYS_AHB_TIMEOUT_EN_ADDRESS(0x%x) ioremap fail!\n",
...
{
	WMT_PLAT_PR_DBG("Turn on reg_VCN33_1_BT in RC mode\n");
#if (COMMON_KERNEL_PMIC_SUPPORT)
	if (g_regmap) {
	...
	if (reg_VCN33_1_BT) {
		...
		if (regulator_enable(reg_VCN33_1_BT))
			WMT_PLAT_PR_DBG("WMT do WIFI PMIC on fail!\n");
	}
}

static VOID consys_hw_vcn33_primary_legacy_mode_enable(VOID)
{
	WMT_PLAT_PR_DBG("Turn on reg_VCN33_1_BT in legacy mode\n");
...
static VOID consys_hw_vcn13_secondary_rc_mode_enable(VOID)
{
	...
	if (reg_VCN13) {
		...
		if (regulator_enable(reg_VCN13))
			WMT_PLAT_PR_DBG("enable VCN13 fail\n");
		else
...
{
#if (COMMON_KERNEL_PMIC_SUPPORT)
	WMT_PLAT_PR_DBG("Turn on VCN18 in RC mode\n");
	if (g_regmap) {
		/*  PMRC_EN[7][6][5][4] HW_OP_EN = 1, HW_OP_CFG = 0 */
		...
		WMT_PLAT_PR_DBG("g_regmap is NULL!!! Please contact WMT owner\n");
#else
	/*  PMRC_EN[7][6][5][4] HW_OP_EN = 1, HW_OP_CFG = 0 */
...
static INT32 consys_hw_vcn18_ctrl(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
		if (consys_is_rc_mode_enable()) {
			...
			if (reg_VCN18) {
				...
				if (regulator_enable(reg_VCN18))
					WMT_PLAT_PR_DBG("enable VCN18 fail\n");
				else
			...
			WMT_PLAT_PR_DBG("Turn on VCN18 in legacy mode\n");
		...
		if (mtk_wcn_consys_get_adie_chipid() == PRIMARY_ADIE) {
			WMT_PLAT_PR_DBG("Turn off VCN33\n");
			if (reg_VCN33_1_BT) {
				if (regulator_disable(reg_VCN33_1_BT))
					WMT_PLAT_PR_DBG("disable VCN33 fail!\n");
			}
		} else if (mtk_wcn_consys_get_adie_chipid() == SECONDARY_ADIE) {
			WMT_PLAT_PR_DBG("Turn off VCN13\n");
			if (reg_VCN13) {
				if (regulator_disable(reg_VCN13))
					WMT_PLAT_PR_DBG("disable VCN13 fail!\n");
			}
		}
		/* delay 300us */
		udelay(300);

		WMT_PLAT_PR_DBG("Turn off VCN18\n");
		if (reg_VCN18) {
			if (regulator_disable(reg_VCN18))
				WMT_PLAT_PR_DBG("disable VCN18 fail!\n");
		}
	}
#endif
	return 0;
...
static INT32 consys_hw_vcn28_ctrl(UINT32 enable)
{
	...
	if (consys_is_rc_mode_enable()) {
		WMT_PLAT_PR_DBG("Configure reg_VCN33_2 in RC mode\n");
		...
		if (enable) {
			WMT_PLAT_PR_DBG("Turn on reg_VCN33_2_WIFI in legacy mode\n");
			...
			if (reg_VCN33_2_WIFI) {
				...
				WMT_PLAT_PR_DBG("turn off vcn33_2_WIFI\n");
			}
		}
	}
#endif
...
static VOID consys_hw_bt_vcn33_disable(VOID)
{
	...
	if (consys_is_rc_mode_enable()) {
		WMT_PLAT_PR_DBG("Do nothing for reg_VCN33_1_BT under RC mode when disable\n");
	} else {
...
static INT32 consys_hw_wifi_vcn33_ctrl(UINT32 enable)
{
	...
	if (enable) {
		if (consys_is_rc_mode_enable()) {
			WMT_PLAT_PR_DBG("Turn on reg_VCN33_1_WIFI in RC mode\n");
			WMT_PLAT_PR_DBG("Turn on reg_VCN33_2_WIFI in RC mode\n");
			consys_hw_wifi_vcn33_rc_mode_enable();
		} else {
			WMT_PLAT_PR_DBG("Turn on reg_VCN33_1_WIFI in legacy mode\n");
		...
		if (consys_is_rc_mode_enable())
			WMT_PLAT_PR_DBG("Do nothing for VCN33_1 under RC mode when disable\n");
		else
		...
		PUINT32 irq_flag)
{
	...
	if (node) {
		...
		*irq_flag = irq_get_trigger_type(*irq_num);
		...
		WMT_PLAT_PR_DBG("[%s] can't find CONSYS compatible node\n", __func__);
		return -1;
	}
...
static INT32 consys_poll_cpupcr_dump(UINT32 times, UINT32 sleep_ms)
{
	...
	WMT_PLAT_PR_DBG("TIME/CPUPCR: %s", str);
	return 0;
}
...
static UINT32 consys_adie_chipid_checking_flow(UINT32 adie)
{
	...
	if (!conn_rf_spi_base) {
		WMT_PLAT_PR_DBG("ioumap 0x180c6000 fail");
		return 0;
	}
...
static INT32 consys_adie_chipid_detect(VOID)
{
	...
	WMT_PLAT_PR_DBG("A-die chip id = %x\n", chipid);
...
static VOID consys_conn2ap_sw_irq_clear(VOID)
{
	...
	if (osal_trylock_unsleepable_lock(&g_pwr_off_lock) == 0) {
		WMT_PLAT_PR_DBG("fail to get pwr off lock\n");
		return;
	}
	...
	if (atomic_read(&g_power_on) == 1) {
		...
		if (ret > 0)
			WMT_PLAT_PR_DBG("CONN2AP_SW_IRQ[27:24] = 0x%x\n", ret);
	}
	...
	if (enable) {
		*addr = ioremap(gConEmiPhyBase + CONSYS_EMI_COREDUMP_OFFSET,
		...
		if (*addr) {
			...
			WMT_PLAT_PR_DBG("EMI mapping fail\n");
			return -1;
		}
	} else {
		if (*addr) {
...
{
	if (!addr) {
		WMT_PLAT_PR_DBG("get virtual address fail\n");
		return -1;
	}
	WMT_PLAT_PR_DBG("Reset EMI(0xF0068000 ~ 0xF0107FFF)\n");
	memset_io(addr, 0, CONSYS_EMI_COREDUMP_MEM_SIZE);
	return 0;
}
...
static INT32 consys_check_reg_readable(VOID)
{
	...
	if (!can_read)
		WMT_PLAT_PR_DBG("connsys clock check fail 0x18007000(0x%x)\n", value);
...
static VOID consys_ic_clock_fail_dump(VOID)
{
	...
	if (addr) {
		...
		WMT_PLAT_PR_DBG("0x180bc000 ioremap fail!\n");
	}
	...
	if (addr) {
		...
		WMT_PLAT_PR_DBG("0x10001B20 ioremap fail!\n");
	}
	...
	if (addr) {
		...
		WMT_PLAT_PR_DBG("0x180c1144 ioremap fail!\n");
	}
	...
	if (addr) {
		...
		WMT_PLAT_PR_DBG("0x1020E804 ioremap fail!\n");
	}
#endif
	WMT_PLAT_PR_DBG("%s length = %d", buffer, osal_strlen(buffer));
}
...
static VOID consys_set_mcif_emi_mpu_protection(MTK_WCN_BOOL enable)
{
	WMT_PLAT_PR_DBG("Setup region 23 for domain 0 as %s\n", enable ? "FORBIDDEN" : "SEC_R_NSEC_R");
