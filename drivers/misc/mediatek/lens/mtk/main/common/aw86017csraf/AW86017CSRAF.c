...
#include "lens_info.h"
#include "AW86017CSRAF.h"

#define AW86017CSRAF_SLAVE_ADDR		(0x18) /* 0x0c: 0x18 >> 1 */
#define AW86017CSRAF_DRIVER_VERSION	"v0.3.0"
 ...
 * I2c read/write
 ******************************************************************************/
static int AW86017CSRAF_WriteRegs(unsigned char a_uAddr, unsigned char *a_uData,
							unsigned int len)
{
	int ret = 0;
	...
	g_pstAF_I2Cclient->addr = (AW86017CSRAF_SLAVE_ADDR >> 1);
...
static int AW86017CSRAF_ReadRegs(unsigned char a_uAddr, unsigned char *a_puData,
...
static inline int AW86017CSRAF_SoftReset(void)
{
	uint8_t val = 0;
	int ret = 0;

	val = AW_SHUTDOWN;
	ret = AW86017CSRAF_WriteRegs(AW_REG_CONTROL, &val, AW_DATA_BYTE_1);
	if (ret < 0) {
...
static inline int AW86017CSRAF_SetAlgoMode(void)
{
	...
	/* enter advance mode */
	ret = AW86017CSRAF_WriteRegs(mode_cmd[0], &mode_cmd[1], AW_DATA_BYTE_1);
	if (ret < 0) {
	...
	/* Identification IC */
	ret = AW86017CSRAF_ReadRegs(AW_REG_CHIP_ID, &chip_id, AW_DATA_BYTE_1);
	if (ret < 0) {
	...
	if (type == AW_BOTTOM_MOTOR) {
		...
		ret = AW86017CSRAF_WriteRegs(AW_REG_ALG_MODE, algo_cfg,
								AW_DATA_BYTE_3);
		if (ret < 0) {
		...
		ret = AW86017CSRAF_ReadRegs(AW_REG_CONTROL, &ctl_val,
								AW_DATA_BYTE_1);
		if (ret < 0) {
		...
		ret = AW86017CSRAF_WriteRegs(AW_REG_CONTROL, &ctl_val,
								AW_DATA_BYTE_1);
		if (ret < 0) {
...
static inline int AW86017CSRAF_InitDrv(void)
{
	int ret = 0;
...
static int AW86017CSRAF_SetPos(unsigned long a_u4Position)
{
	int ret = 0;
...
static inline int AW86017CSRAF_MoveVCM(unsigned long a_u4Position)
{
	AW_LOGI("Start.");
	...
	if (*g_pAF_Opened == 2) {
		if (AW86017CSRAF_SetPos(a_u4Position) == 0) {
			spin_lock(g_pAF_SpinLock);
...
static inline int AW86017CSRAF_SetMacro(unsigned long a_u4Param)
{
	AW_LOGI("Start.");
...
static inline int AW86017CSRAF_SetInf(unsigned long a_u4Param)
{
	AW_LOGI("Start.");
...
static inline int AW86017CSRAF_GetVCMInfo(
...
long AW86017CSRAF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
							unsigned long a_u4Param)
{
	...
	case AFIOC_G_MOTORINFO:
		ret = AW86017CSRAF_GetVCMInfo(
				(__user struct stAF_MotorInfo *) (a_u4Param));
		break;
	case AFIOC_T_MOVETO:
		ret = AW86017CSRAF_MoveVCM(a_u4Param);
		break;
	case AFIOC_T_SETMACROPOS:
		ret = AW86017CSRAF_SetMacro(a_u4Param);
		break;
	case AFIOC_T_SETINFPOS:
		ret = AW86017CSRAF_SetInf(a_u4Param);
		break;
	default:
		AW_LOGE("NO CMD!.");
		ret = -EPERM;
...
int AW86017CSRAF_Release(struct inode *a_pInode, struct file *a_pstFile)
{
	int ret = 0;
...
int AW86017CSRAF_PowerDown(struct i2c_client *pstAF_I2Cclient, int *pAF_Opened)
{
	...
	/* enter PD mode, PD = 1; */
	ret = AW86017CSRAF_WriteRegs(pd_cmd[0], &pd_cmd[1], AW_DATA_BYTE_1);
	if (ret < 0) {
...
int AW86017CSRAF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int AW86017CSRAF_GetFileName(uint8_t *pFileName)
{
...
MODULE_DESCRIPTION("AW86017CSRAF VCM Driver");
