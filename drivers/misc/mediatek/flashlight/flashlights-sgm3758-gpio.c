...
static int sgm3758_enable(void)
{
	...
	//if (sgm3758_level > 1){
		printk("sgm3758_enable-----start\n");
		sgm3758_pinctrl_set(HWEN_PIN, 1);
		printk("sgm3758_enable-----end\n");
	//}
...
{
	printk("sgm3758_disable-----start\n");
	return sgm3758_pinctrl_set(HWEN_PIN, 0);
	printk("sgm3758_disable-----end\n");
}
...
static int sgm3758_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_SET_ONOFF:
		printk("sgm3758--111-FLASH_IOC_SET_ONOFF(%d): %d\n",
				channel, (int)fl_arg->arg);
		if (fl_arg->arg == 1) {
			...
			printk("sgm3758--222-FLASH_IOC_SET_ONOFF\n");
			sgm3758_enable();
		} else {
			sgm3758_disable();
