...
retry:
	...
	if (header->marker != MESSAGE_MARKER) {
		if (!retry) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"Incorrect header marker (0x%02x)(0x%02x)(0x%02x)(0x%02x)\n",
					header->marker,header->code,header->length[0],header->length[1]);
		}
		UNLOCK_BUFFER(tcm_hcd->in);
		retval = -ENXIO;
		if (retry) {
...
/*prize add by lvyuanchuan 20220214 start */
	...
	strcpy(current_tp_info.chip, "td4160 fw:001");
...
static int ovt_tcm_probe(struct platform_device *pdev)
{
	...
	for (idx = 0; idx < ARRAY_SIZE(attrs); idx++) {
		retval = sysfs_create_file(&tcm_hcd->pdev->dev.kobj,
				&(*attrs[idx]).attr);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
