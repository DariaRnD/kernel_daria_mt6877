		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&gts->mutex);
	sitronix_mode_switch(modeID, modeValue);
	mutex_unlock(&gts->mutex);
...
static ssize_t sitronix_proc_grip_write(struct file *filp, const char *buff, size_t size, loff_t *pPos)
{
	...
	if (strcmp(cmd, sitronix_proc_table[6].on_cmd) == 0)
		sitronix_mode_switch(ST_MODE_GRIP, true);
	else if (strcmp(cmd, sitronix_proc_table[6].off_cmd) == 0)
		sitronix_mode_switch(ST_MODE_GRIP, false);
	else
		return -EIO;
...
static ssize_t sitronix_proc_grip_rotate_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{

	uint32_t len = 0;

	len = sitronix_copy_to_user("ST_MODE_GRIP_ROTATE", gts->mode_flag[ST_MODE_GRIP_ROTATE], buff);
	*pPos = len;

	return 0;
}

static ssize_t sitronix_proc_grip_rotate_write(struct file *filp, const char *buff, size_t size, loff_t *pPos)
{
	char cmd[10] = { 0 };
	...
	if (strcmp(cmd, sitronix_proc_table[7].on_cmd) == 0)
		sitronix_mode_switch(ST_MODE_GRIP_ROTATE, true);
	else if (strcmp(cmd, sitronix_proc_table[7].off_cmd) == 0)
		sitronix_mode_switch(ST_MODE_GRIP_ROTATE, false);
	else
		return -EIO;
...
struct file_operations proc_grip_rotate_fops = {
	.owner = THIS_MODULE,
	.write = sitronix_proc_grip_rotate_write,
	.read = sitronix_proc_grip_rotate_read,
};
...
sitronix_proc_node_t sitronix_proc_table[] = {
	...
	{"grip", NULL, &proc_grip_fops, true, "on", "off"},
	{"grip_rotate", NULL, &proc_grip_rotate_fops, true, "on", "off"},
