...
#define HYN_TP_OPEN_VALUE_MAX          	    1400
#define HYN_TP_OPEN_VALUE_MIN          	    240
#define HYN_TP_OPEN_DRV_VALUE_MIN         400
#define HYN_TP_OPEN_DROP_RATIO               20
...
uint16_t open_value_max;
uint16_t open_value_min;
uint16_t open_drv_value_min;
uint8_t  open_drop_ratio;
uint8_t  open_tx_drop_ratio;
...
static ssize_t hyn_tprwreg_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	...
	buf_cmd[0] = 0xD0;
	buf_cmd[1] = 0x4F;
	ret = cst3xx_i2c_read_register(hyn_ts_data->client, buf_cmd, 6);
	if (ret < 0) return -1;

   	num_read_chars = snprintf(buf, 128, "buf0:%d,buf1:%d,buf2:%d,buf3:%d,buf4:%d,buf5:%d .\n",buf_cmd[0],buf_cmd[1], buf_cmd[2],buf_cmd[3],buf_cmd[4],buf_cmd[5]);
