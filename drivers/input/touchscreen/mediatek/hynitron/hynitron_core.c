...
#ifdef CONFIG_PRIZE_HARDWARE_INFO
int read_fw_project_version(struct hynitron_ts_data* hyn, unsigned int *fw_version,unsigned int *proj_version)
{
	unsigned int chip_version = 0;
	unsigned int project_version = 0;
	u8 buf1[10];
	int ret;
	
	buf1[0] = 0xD1;
	buf1[1] = 0x01;
	ret = cst3xx_i2c_write(hyn->client, buf1, 2);
	if (ret < 0) {
		*fw_version = 0;
		*proj_version = 0;
		return -1;
	...
	buf1[0] = 0xD2;
	buf1[1] = 0x04;
	ret = cst3xx_i2c_read_register(hyn->client, buf1, 4);
	if (ret < 0) {
		*fw_version = 0;
		*proj_version = 0;
		return -1;
...
static int hyn_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    hyn_irq_enable();
#ifdef CONFIG_PRIZE_HARDWARE_INFO
	read_fw_project_version(ts_data,&fw,&pri_id);
	sprintf(current_tp_info.chip, "FW:0x%08x",fw);
	sprintf(current_tp_info.id,"0x%04x",pri_id);
	strcpy(current_tp_info.vendor, "CST14055");
