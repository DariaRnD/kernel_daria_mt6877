...
#ifdef CONFIG_OF
static const struct of_device_id gt1xx_dt_match[] = { {.compatible = "mediatek,goodix_touch"}, {},};
#endif
...
#endif
static int of_get_gt1xx_platform_data(struct device *dev)
{
#if defined(CONFIG_OF) && !defined(CONFIG_GTP_USE_PINCTRL)
	if (dev->of_node) {
		const struct of_device_id *match;

		match = of_match_device(of_match_ptr(gt1xx_dt_match), dev);
		if (!match) {
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	of_get_gt1xx_platform_data(&client->dev);
	err = gtp_get_gpio_res();
	if (err < 0)
