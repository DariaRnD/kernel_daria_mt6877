...
/* begin, prize-lifenfen-20180104, add for touchpanel hardware info */
	...
	memset(buffer, 0, sizeof(buffer));
	r = goodix_i2c_read(dev, 0x6f78, buffer, 1);
    ...
    sprintf(current_tp_info.id,"%02x cfg:0x%02X",version->sensor_id,buffer[0]);
		...
		struct goodix_ts_config *config)
{
	...
	printk("cur ver:%02xh config ver:%02xh,size:%d",
		buffer[0],
		config->data[0],
		config->length);
		
	if(buffer[0] >= config->data[0]){
		printk("prize liaojie curr firmware:0x%02X  bin firmware:0x%02X,so don't need update\n",buffer[0],config->data[0]);
		return 0;
	}
	printk("prize liaojie curr firmware:0x%02X  bin firmware:0x%02X,so going to update\n",buffer[0],config->data[0]);
