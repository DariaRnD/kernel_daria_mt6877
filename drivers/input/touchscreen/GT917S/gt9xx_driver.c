...
//prize-wangyongsheng-20210122-start
extern struct hardware_info current_touch_info;
//prize-wangyongsheng-20210122-end
#endif 
...
static s32 gtp_send_cfg(struct i2c_client *client)
{
		...
		for (retry = 0; retry < 5; retry++) {
			...
			if (ret > 0)
			{
				GTP_INFO("init_panel, gtp_send_cfg ok\n !");
				break;
			}
...
s32 gtp_init_panel(struct i2c_client *client)
{
	...
	pnl_init_error = 0;
	if ((!cfg_info_len[0]) && (!cfg_info_len[2]) &&
...
#else
	{
		...
		if (ret == SUCCESS) {
			GTP_INFO("CFG_CONFIG_GROUP%d Config Version: %d, 0x%02X; IC Config Version: %d, 0x%02X",
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
		...
		while (retry++ < 5) {
			...
			if ((!ret) && ((hw_info == 0x00900802)||(hw_info == 9441283))){
				if(hw_info == 0x00900802)
					hw_chip_type = 1;//GT917S
				else if(hw_info == 0x00901003)
					hw_chip_type = 2;//GT5688
...
/*prize add by zhaopengge 20201113--------start*/	
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
	gtp_i2c_read_dbl_check(client, GTP_REG_SENSOR_ID, &sensor_id, 1);
	gtp_i2c_read_dbl_check(client, GTP_REG_CONFIG_DATA, &ctp_fw_version, 1);

	if(hw_chip_type==1)
		sprintf(current_tp_info.chip,"GT917S fw_version:0x%02x", ctp_fw_version);
	else if(hw_chip_type==2)
		sprintf(current_tp_info.chip,"GT5688 fw_version:0x%02x", ctp_fw_version);
	else	
		sprintf(current_tp_info.chip,"xxxx fw_version:0x%02x", ctp_fw_version);
    sprintf(current_tp_info.id,"0x%04x",sensor_id);
    strcpy(current_tp_info.vendor,"goodix");
    sprintf(current_tp_info.more,"%d*%d",1080,1920);
//prize-wangyongsheng-20210122-start	
	if(hw_chip_type==1)
		sprintf(current_touch_info.chip,"GT917S");
	else if(hw_chip_type==2)
		sprintf(current_touch_info.chip,"GT5688");
	else
		sprintf(current_touch_info.chip,"XXXX");
    sprintf(current_touch_info.id,"0x%04x",sensor_id);
    strcpy(current_touch_info.vendor,"Goodix");
    sprintf(current_touch_info.more,"0x%02x", ctp_fw_version);	
//prize-wangyongsheng-20210122-end	
#endif
/*prize add by zhaopengge 20201113--------end*/
