...
#include "sgm41512v1.h"
#define SGM41512_DRV_VERSION	"1.0.1_SGM"

#ifndef SGM41512_IINDPM_SET
#define SGM41512_IINDPM_SET
#endif

#ifdef SGM41512_IINDPM_SET
#define 	POWER_SUPPLY_TYPE_USB_CURRENT				500000
#define		POWER_SUPPLY_TYPE_USB_CDP_CURRENT			1500000
#define		POWER_SUPPLY_TYPE_USB_DCP_CURRENT			2000000
#define		POWER_SUPPLY_TYPE_USB_DCP_CURRENT_2			1500000

static int current_set = 0;
static struct delayed_work delay_work;
struct sgm41512_chip *g_chip = NULL;
static unsigned short set_count = 5;
extern void sgm41512_iindpm_set(int on);
...
#endif
int sgm41512_charger_get_type(struct sgm41512_chip *info);
...
struct sgm41512_chip {
	...
	/* prize hanjiuping 20211008 added for otg regulator */
...
static int __sgm41512_enable_chg(struct sgm41512_chip *chip, bool en)
{
	...
	pr_err("gezi  en_last=%d .en = %d..\n",en_last,en);
	if((chip->psy) && (en_last != en)){
...
static int __sgm41512_dump_registers(struct sgm41512_chip *chip)
{
	...
	u8 regbuf[ARRAY_SIZE(sgm41512_reg_addr)] = {};
	...
	if (1){// ic_normal == false) {
		...
		dev_info(chip->dev, "DUMP:%02X %02X %02X %02X %02X %02X %02X %02X %02X "\
			"%02X %02X %02X\n",
			regbuf[0],regbuf[1],regbuf[2],regbuf[3],regbuf[4],regbuf[5],
			regbuf[6],regbuf[7],regbuf[8],regbuf[9],regbuf[10],regbuf[11]);
	}

	dev_info(chip->dev, "%s MIVR = %dmV, AICR = %dmA VINDPM:%d IINDPM:%d\n",
		 __func__, mivr / 1000, aicr / 1000,
		 (regbuf[SGM41512_REG_0A]&SGM41512_ST_MIVR_MASK) >> 6,
		 (regbuf[SGM41512_REG_0A]&SGM41512_ST_AICR_MASK) >> 5);

	dev_info(chip->dev, "%s chg_en = %d, chg_stat = %d, vbus_stat = %d ic_stat = %d\n",
...
retry:
	...
	dev_info(chip->dev, "%s REG_ICSTAT_%02d:%x ret:%d type:%d\n", __func__,
			SGM41512_REG_ICSTAT, port, ret, chip->port);
	...
	if(chip->chg_type == CHARGER_UNKNOWN && attach)	{
		dev_info(chip->dev, "%s chg_type detect err %d\n", __func__,retry_cnt);
		sgm41512_force_iildet_en(chip,true);
		msleep(50);
		retry_cnt++;
...
{
	dev_info(chip->dev, "%s\n", __func__);
	//return __sgm41513_kick_wdt(chip);
	return 0;
}
...
*/
	...
	if (evt[SGM41512_ST_FAULT] & SGM41512_WDFAULT_MASK){
		sgm41512_wdt_irq_handler(chip);
	}
#if 0
...
#ifdef SGM41512_IINDPM_SET
void sgm41512_iindpm_set(int on)
{
	int charge_type = POWER_SUPPLY_TYPE_UNKNOWN;

	printk("gezi sgm41512_iindpm_set...\n");

	if(on)
	{
		charge_type = sgm41512_charger_get_type(g_chip);

		printk("gezi sgm41512_iindpm_set..charge_type = %d.\n",charge_type);

		if(charge_type == POWER_SUPPLY_TYPE_USB_DCP)
		{
			if(g_charge_is_screen_on){
				current_set = POWER_SUPPLY_TYPE_USB_DCP_CURRENT_2;
			}
			else{
				current_set = POWER_SUPPLY_TYPE_USB_DCP_CURRENT;
			}
		}
		else if(charge_type == POWER_SUPPLY_TYPE_USB_CDP)
		{
			current_set = POWER_SUPPLY_TYPE_USB_DCP_CURRENT;
		}
		else
		{
			current_set = POWER_SUPPLY_TYPE_USB_CURRENT;
		}
		schedule_delayed_work(&delay_work, msecs_to_jiffies(200));
	}
	else
	{
		printk("gezi cancel_delayed_work...\n");
		set_count = 5;
...
/* prize hanjiuping 20211025 modified for workaround of cc misssing charge function start */
...
{
	int ret = 0;
	bool is_pp_enabled = false;
	static bool pp_enabled_in_chgen = false;
	struct sgm41512_chip *chip = dev_get_drvdata(&chg_dev->dev);
	...
	if (en) {
		...
		sgm41512_is_hz_enabled(chg_dev, &is_pp_enabled);
		if (!is_pp_enabled) {
			sgm41512_enable_hz(chg_dev, true);
			pp_enabled_in_chgen = true;
		}
	}
	...
	if (!en) {
		...
		if (pp_enabled_in_chgen) {
			sgm41512_enable_hz(chg_dev, false);
			pp_enabled_in_chgen = false;
		}
	}
...
/* prize hanjiuping 20211008 added for otg regulator start */
static int sgm41512_enable_vbus(struct regulator_dev *rdev)
{
	struct sgm41512_chip *chip = rdev_get_drvdata(rdev);

	dev_info(chip->dev, "%s\n", __func__);
	return sgm41512_enable_otg(chip->chg_dev, true);
}

static int sgm41512_disable_vbus(struct regulator_dev *rdev)
{
	struct sgm41512_chip *chip = rdev_get_drvdata(rdev);

	dev_info(chip->dev, "%s\n", __func__);
	return sgm41512_enable_otg(chip->chg_dev, false);
}

static int sgm41512_is_enabled_vbus(struct regulator_dev *rdev)
{
	int ret      = 0;
	u8 vbus_stat = 0;
	struct sgm41512_chip *chip = rdev_get_drvdata(rdev);
	...
	ret = __sgm41512_get_vbus_stat(chip, &vbus_stat);
	if (ret < 0) {
		dev_info(chip->dev, "%s get vbus stat failed, return %d\n",
			__func__, ret);
		return ret;
	}

/* OTG is 0b'111(VBUS_STAT[2:0]) at D[7:5] of REG08 */
#define SGM41512_VBUSSTAT_OTG 0x07

	return vbus_stat == SGM41512_VBUSSTAT_OTG;
}

static const struct regulator_ops sgm41512_vbus_ops = {
	.enable = sgm41512_enable_vbus,
	.disable = sgm41512_disable_vbus,
	.is_enabled = sgm41512_is_enabled_vbus,
};

static const struct regulator_desc sgm41512_otg_rdesc = {
	.of_match = "usb-otg-vbus",
	.name = "usb-otg-vbus",
	.ops = &sgm41512_vbus_ops,
	.owner = THIS_MODULE,
	.type = REGULATOR_VOLTAGE,
	.fixed_uV = 5000000,
	.n_voltages = 1,
};
/* prize hanjiuping 20211008 added for otg regulator end */
...
static int sgm41512_charger_get_online(struct sgm41512_chip *info,bool *val)
{
...
static int sgm41512_charger_get_usb_type(struct sgm41512_chip *info)
{
	union power_supply_propval prop;
	int ret = 0;
...
{
	struct sgm41512_chip *chip = power_supply_get_drvdata(psy);

	enum sgm41512_ic_stat chg_stat = SGM41512_ICSTAT_SLEEP;
	...
	case POWER_SUPPLY_PROP_ONLINE:
		ret = sgm41512_charger_get_online(chip, &pwr_rdy);
		val->intval = pwr_rdy;
		break;
	case POWER_SUPPLY_PROP_STATUS:
		ret = sgm41512_charger_get_online(chip, &pwr_rdy);
		ret = sgm41512_is_charging_enabled(chip->chg_dev, &chg_en);
		ret = __sgm41512_get_ic_stat(chip, &chg_stat);

		pr_err("gezi pwr_rdy = %d,chg_en = %d,chg_stat = %d\n",pwr_rdy,chg_en,chg_stat);
		...
		switch (chg_stat) {
		case SGM41512_ICSTAT_SLEEP:
			if(chg_en)
				val->intval = POWER_SUPPLY_STATUS_CHARGING;
			else
				val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
			break;
		case SGM41512_ICSTAT_PRECHG:
		case SGM41512_ICSTAT_FASTCHG:
			if (chg_en)
				val->intval = POWER_SUPPLY_STATUS_CHARGING;
			else
				val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
			break;
		case SGM41512_ICSTAT_CHGDONE:
	...
	case POWER_SUPPLY_PROP_TYPE:
		val->intval = sgm41512_charger_get_type(chip);
		break;
	case POWER_SUPPLY_PROP_USB_TYPE:
		val->intval = sgm41512_charger_get_usb_type(chip);
		break;
	default:
		ret = -ENODATA;
	}
...
#ifdef SGM41512_IINDPM_SET
static void sgm41512_iindpm_set_work(struct work_struct *work)
{

	printk("gezi sgm41512_iindpm_set_work...set_count = %d\n",set_count);

	__sgm41512_set_aicr(g_chip,current_set);
	set_count--;
	if(set_count > 0){
			...
			const struct i2c_device_id *id)
{
	...
	charger_cfg.supplied_to = sgm41512_charger_supplied_to;
	charger_cfg.num_supplicants = ARRAY_SIZE(sgm41512_charger_supplied_to);
	...
	chip->otg_rdev = devm_regulator_register(chip->dev,
						&sgm41512_otg_rdesc, &config);
...
#ifdef SGM41512_IINDPM_SET
	g_chip = chip;
	INIT_DELAYED_WORK(&delay_work, sgm41512_iindpm_set_work);
#endif
