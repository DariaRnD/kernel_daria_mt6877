...
#define FG_DBG(fmt, ...) \
	do { \
		if (FG_DBG_EN) \
			pr_info("[CW221X][%s][%d] " fmt, __func__,__LINE__, ##__VA_ARGS__); \
	} while (0)
...
/*prize modified by lvyuanchuan for Optimization of low-voltage switching processes,20221130 start*/
#define FG_SHUTDOWN_VOL					(3400)
/*0.1*/
#define SHUTDOWN_REMAINDER		  (10)
