...
static int __maybe_unused aw_cali_write_cali_re_to_file(int32_t cali_re, int channel)
{
	struct file *fp = NULL;
	char buf[50] = { 0 };
	loff_t pos = 0;
...
{
#ifdef AW_CALI_STORE_EXAMPLE
	/* prize modified for moving writing re to nvram to HAL layer
	   to comply with the system selinux rules start */
	return 0;
	/* return aw_cali_write_cali_re_to_file(cali_re, channel); */
	/* prize modified for moving writing re to nvram to HAL layer end */
#else
	return 0;
#endif
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (is_single_cali) {
		len += snprintf(buf+len, PAGE_SIZE-len, "dev[%d]:%u mOhms",
	...
	struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len,
		"re_min value: [%d]", range_buf[RE_MIN_FLAG]);
	len += snprintf(buf + len, PAGE_SIZE - len,
		"re_max value: [%d]", range_buf[RE_MAX_FLAG]);
